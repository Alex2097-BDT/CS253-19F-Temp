...
#include <map>
using namespace std;
#ifndef WEEK5_WORDS2_H
#define WEEK5_WORDS2_H
bool isAlpha(char c){
...
vector<string> filter_std(vector<string> candidate_terms){
    ...
    for(auto pt = candidate_terms.begin();pt!=candidate_terms.end();pt++){
        if(strcmp(pt->c_str(), s.c_str())==0 || strcmp(pt->c_str(), d.c_str())==0){
