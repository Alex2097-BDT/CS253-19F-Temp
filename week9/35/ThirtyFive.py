...
overall_prediction = np.zeros(6398)

while end_id < len(indices):
    batch_x = encode_binary(indices[start_id:end_id])
    ...
    overall_prediction = np.sum([overall_prediction, prediction], axis=0)
    start_id = start_id + batch_size
    end_id = end_id + batch_size

batch_x = encode_binary(indices[start_id:len(indices)])
intermediate_model = Model(inputs=model.input, outputs=[l.output for l in model.layers])
preds = intermediate_model.predict(batch_x) # outputs a list of 4 arrays
...
for w, c in sorted(list(zip(uniqs, overall_prediction)), key = operator.itemgetter(1), reverse=True)[:25]:
    print(w + " - " + str(c))
